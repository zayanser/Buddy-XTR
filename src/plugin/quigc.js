import config from '../../config.cjs';

const gcEvent = async (m, Matrix) => {
    const prefix = config.PREFIX;
    
    // Early return if message doesn't start with prefix
    if (!m.body.startsWith(prefix)) return;
    
    const cmd = m.body.slice(prefix.length).split(' ')[0].toLowerCase();
    const text = m.body.slice(prefix.length + cmd.length).trim();

    if (cmd === 'quizgc') {
        try {
            // Validate group context
            if (!m.isGroup) {
                return await m.reply("*🎮 THIS QUIZ GAME CAN ONLY BE PLAYED IN GROUPS*");
            }

            // Add reaction to show the bot is processing
            await Matrix.sendMessage(m.from, { react: { text: "⏳", key: m.key } });

            // Large collection of questions (100+)
            const quizQuestions = [
                // General Knowledge
                { question: "What is the capital of France?", answer: "Paris" },
                { question: "How many continents are there on Earth?", answer: "7" },
                { question: "What is the largest planet in our solar system?", answer: "Jupiter" },
                { question: "Who painted the Mona Lisa?", answer: "Leonardo da Vinci" },
                { question: "What is the chemical symbol for gold?", answer: "Au" },
                { question: "Which country is home to the kangaroo?", answer: "Australia" },
                { question: "What is the tallest mountain in the world?", answer: "Mount Everest" },
                { question: "Which planet is known as the Red Planet?", answer: "Mars" },
                { question: "What is the largest ocean on Earth?", answer: "Pacific Ocean" },
                { question: "Who wrote 'Romeo and Juliet'?", answer: "William Shakespeare" },
                
                // Science and Technology
                { question: "What is the hardest natural substance on Earth?", answer: "Diamond" },
                { question: "Which gas do plants absorb from the atmosphere?", answer: "Carbon dioxide" },
                { question: "What is the main component of the Sun?", answer: "Hydrogen" },
                { question: "Which inventor is associated with the light bulb?", answer: "Thomas Edison" },
                { question: "What does 'www' stand for in a website address?", answer: "World Wide Web" },
                { question: "Which element has the atomic number 1?", answer: "Hydrogen" },
                { question: "What is the study of fossils called?", answer: "Paleontology" },
                { question: "Which planet has the most moons?", answer: "Saturn" },
                { question: "What is the human body's largest organ?", answer: "Skin" },
                { question: "What is the speed of light in a vacuum?", answer: "299,792 km/s" },
                
                // History
                { question: "In which year did World War II end?", answer: "1945" },
                { question: "Who was the first president of the United States?", answer: "George Washington" },
                { question: "Which ancient civilization built the pyramids?", answer: "Ancient Egyptians" },
                { question: "Who discovered America in 1492?", answer: "Christopher Columbus" },
                { question: "Which empire was ruled by Julius Caesar?", answer: "Roman Empire" },
                { question: "When did the Berlin Wall fall?", answer: "1989" },
                { question: "Who was the first man to walk on the moon?", answer: "Neil Armstrong" },
                { question: "Which country gifted the Statue of Liberty to the US?", answer: "France" },
                { question: "Who was the first female prime minister of the UK?", answer: "Margaret Thatcher" },
                { question: "In which year did the Titanic sink?", answer: "1912" },
                
                // Entertainment
                { question: "Who played Jack in Titanic?", answer: "Leonardo DiCaprio" },
                { question: "Which fictional wizard is known for attending Hogwarts?", answer: "Harry Potter" },
                { question: "What is the highest-grossing film of all time?", answer: "Avatar" },
                { question: "Which singer is known as the 'Queen of Pop'?", answer: "Madonna" },
                { question: "Who directed the movie 'Jurassic Park'?", answer: "Steven Spielberg" },
                { question: "Which band sang 'Bohemian Rhapsody'?", answer: "Queen" },
                { question: "What is the name of the fictional country in Black Panther?", answer: "Wakanda" },
                { question: "Who is the main character in 'The Hunger Games'?", answer: "Katniss Everdeen" },
                { question: "Which cartoon character lives in a pineapple under the sea?", answer: "SpongeBob SquarePants" },
                { question: "What is the real name of Iron Man?", answer: "Tony Stark" },
                
                // Sports
                { question: "Which country won the 2022 FIFA World Cup?", answer: "Argentina" },
                { question: "How many players are on a basketball team?", answer: "5" },
                { question: "What is the national sport of Japan?", answer: "Sumo wrestling" },
                { question: "Which athlete is known as 'The Greatest' in boxing?", answer: "Muhammad Ali" },
                { question: "In which sport would you perform a slam dunk?", answer: "Basketball" },
                { question: "How many rings are on the Olympic flag?", answer: "5" },
                { question: "Which country invented the sport of golf?", answer: "Scotland" },
                { question: "What is the maximum score in a single frame of bowling?", answer: "30" },
                { question: "Which tennis player has won the most Grand Slam titles?", answer: "Novak Djokovic" },
                { question: "What color is the jersey of the Tour de France leader?", answer: "Yellow" },
                
                // Geography
                { question: "Which river is the longest in the world?", answer: "Nile" },
                { question: "What is the smallest country in the world?", answer: "Vatican City" },
                { question: "Which desert is the largest in the world?", answer: "Sahara" },
                { question: "What is the capital of Canada?", answer: "Ottawa" },
                { question: "Which country has the most time zones?", answer: "France" },
                { question: "What is the official language of Brazil?", answer: "Portuguese" },
                { question: "Which US state is known as the Sunshine State?", answer: "Florida" },
                { question: "Which continent is the most populous?", answer: "Asia" },
                { question: "What is the currency of Japan?", answer: "Yen" },
                { question: "Which African country was formerly known as Abyssinia?", answer: "Ethiopia" },
                
                // Animals
                { question: "What is the fastest land animal?", answer: "Cheetah" },
                { question: "Which mammal can fly?", answer: "Bat" },
                { question: "What is the largest species of shark?", answer: "Whale shark" },
                { question: "How many legs does a spider have?", answer: "8" },
                { question: "Which bird is the symbol of peace?", answer: "Dove" },
                { question: "What is the only mammal capable of true flight?", answer: "Bat" },
                { question: "Which animal has the longest lifespan?", answer: "Greenland shark" },
                { question: "What is a group of lions called?", answer: "Pride" },
                { question: "Which animal is known as the 'ship of the desert'?", answer: "Camel" },
                { question: "What color is a polar bear's skin?", answer: "Black" },
                
                // Food and Drink
                { question: "Which country produces the most coffee?", answer: "Brazil" },
                { question: "What is the main ingredient in hummus?", answer: "Chickpeas" },
                { question: "Which fruit is known as the 'king of fruits'?", answer: "Mango" },
                { question: "What is the national dish of Spain?", answer: "Paella" },
                { question: "From which country does pizza originate?", answer: "Italy" },
                { question: "What is the most consumed beverage in the world after water?", answer: "Tea" },
                { question: "Which nut is used to make marzipan?", answer: "Almond" },
                { question: "What is the primary ingredient in traditional Japanese miso soup?", answer: "Soybeans" },
                { question: "Which country is the largest producer of chocolate?", answer: "Ivory Coast" },
                { question: "What is the world's most expensive spice?", answer: "Saffron" },
                
                // Literature
                { question: "Who wrote '1984'?", answer: "George Orwell" },
                { question: "What is the best-selling book of all time?", answer: "Bible" },
                { question: "Which Shakespeare play features the characters Romeo and Juliet?", answer: "Romeo and Juliet" },
                { question: "Who wrote 'Pride and Prejudice'?", answer: "Jane Austen" },
                { question: "What is the name of the wizard in 'The Lord of the Rings'?", answer: "Gandalf" },
                { question: "Which novel begins with 'Call me Ishmael'?", answer: "Moby-Dick" },
                { question: "Who wrote 'The Great Gatsby'?", answer: "F. Scott Fitzgerald" },
                { question: "What is the name of Harry Potter's owl?", answer: "Hedwig" },
                { question: "Which author created Sherlock Holmes?", answer: "Arthur Conan Doyle" },
                { question: "What is the first book in the 'Chronicles of Narnia' series?", answer: "The Lion, the Witch and the Wardrobe" },
                
                // Mixed Bag
                { question: "How many colors are in a rainbow?", answer: "7" },
                { question: "What is the name of the fairy in Peter Pan?", answer: "Tinker Bell" },
                { question: "Which planet is closest to the Sun?", answer: "Mercury" },
                { question: "What is the largest species of penguin?", answer: "Emperor penguin" },
                { question: "Which language has the most native speakers?", answer: "Mandarin Chinese" },
                { question: "What is the currency of the United Kingdom?", answer: "Pound sterling" },
                { question: "How many bones are in the adult human body?", answer: "206" },
                { question: "Which country is shaped like a boot?", answer: "Italy" },
                { question: "What is the capital of New Zealand?", answer: "Wellington" },
                { question: "Which musical instrument has 88 keys?", answer: "Piano" }
            ];

            // Select random question
            const randomQuestion = quizQuestions[Math.floor(Math.random() * quizQuestions.length)];
            
            // Send question as forwarded message
            const questionMsg = await Matrix.sendMessage(
                m.from, 
                { 
                    text: `📝 *QUIZ QUESTION:*\n\n${randomQuestion.question}\n\nYou have 30 seconds to answer!`, 
                    forward: true,
                    forwarded: 5
                }, 
                { quoted: m }
            );

            // Add reaction to the question
            await Matrix.sendMessage(m.from, { react: { text: "❓", key: questionMsg.key } });

            // Countdown function
            let seconds = 30;
            const progressBarLength = 15;
            
            const countdownMsg = await Matrix.sendMessage(
                m.from,
                {
                    text: `⏳ Time remaining: ${seconds}s\n[${'█'.repeat(progressBarLength)}]`,
                    forward: true,
                    forwarded: 5
                }
            );

            const countdownInterval = setInterval(async () => {
                seconds--;
                const progress = Math.round((seconds / 30) * progressBarLength);
                const progressBar = '█'.repeat(progress) + '░'.repeat(progressBarLength - progress);
                
                try {
                    await Matrix.sendMessage(
                        m.from,
                        {
                            text: `⏳ Time remaining: ${seconds}s\n[${progressBar}]`,
                            edit: countdownMsg.key
                        }
                    );
                } catch (e) {
                    console.error("Error updating countdown:", e);
                }

                if (seconds <= 0) {
                    clearInterval(countdownInterval);
                    // Send answer as forwarded message
                    const answerMsg = await Matrix.sendMessage(
                        m.from,
                        {
                            text: `✅ *CORRECT ANSWER:*\n\n${randomQuestion.answer}`,
                            forward: true,
                            forwarded: 5
                        }
                    );
                    // Add reaction to the answer
                    await Matrix.sendMessage(m.from, { react: { text: "🎯", key: answerMsg.key } });
                }
            }, 1000);

        } catch (error) {
            console.error("Error in quiz command:", error);
            await Matrix.sendMessage(
                m.from, 
                { text: "⚠️ *An error occurred while starting the quiz*" }, 
                { quoted: m }
            );
            // Add error reaction
            await Matrix.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        }
    }
};

export default gcEvent;
